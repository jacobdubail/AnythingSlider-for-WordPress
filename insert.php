...
function jtd_anythingslider_insert_button($context){
	...
	$out       = '<a href="#TB_inline?width=660&inlineId=anythingSlider_insert_slideshow" class="thickbox" title="Add a Slideshow"><img src="'.$image_btn.'" alt="Add a Slideshow" /></a>';
...
?>
	<script>
		function InsertSlideshow(){
		    ...
		    win.send_to_editor("[anything_slides " + cat_id + " width=" + width + " height=" + height + " " + navFormat + "]");
		}
	</script>
...
<div class="wrap">
	...
	<table class="form-table"><tbody>
		...
		<tr class="form-field">
			...
			<td><input type="number" id="width" value="<?php echo $options['width']; ?>" /></td>
			<th scope="row"><em>Custom width for this slideshow?</em></th>
		</tr>
		
		<tr class="form-field">
			<th scope="row"><label for="height">Height</label></th>
			<td><input type="number" id="height" value="<?php echo $options['height']; ?>" /></td>
